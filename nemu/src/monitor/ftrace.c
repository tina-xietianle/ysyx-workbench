...
void print_func(vaddr_t addr,vaddr_t next,int is_ret){
  int temp_dnpc=0;  //用来打印被调用函数的位置变量
  int temp_pc=0;    //用来打印当前函数的位置变量
  for(temp_dnpc=0; temp_dnpc<symtab_num; temp_dnpc++){
    if(symtab[temp_dnpc].st_info == 0x12){    //0x12即STT_FUNC，通过打印出符号表的数字，与elf文件对应，找到STT_FUNC是0x12
      if(next>=symtab[temp_dnpc].st_value && next<symtab[temp_dnpc].st_value+symtab[temp_dnpc].st_size)
        break;//若地址落在【value,value+size】区间内，且属于FUNC，则找到被调用函数的位置
    }
  }
//遍历符号表，找到被调用函数的位置
  for(temp_pc=0; temp_pc<symtab_num; temp_pc++){
    if(symtab[temp_pc].st_info == 0x12){
      if(addr>=symtab[temp_pc].st_value && addr<symtab[temp_pc].st_value+symtab[temp_pc].st_size)
        break;
    }
  }
//遍历符号表，找到当前函数的位置
  if(is_ret){  //如果是返回指令
    printf("\033[0;36m0x%0x:\033[0m",addr);//打印青色的
    for(int i=0;i<=print_depth;i++)
      printf(" ");
    printf("\033[0;36m[%s]ret  [%s]\033[0m\n",strtab+symtab[temp_pc].st_name,strtab+symtab[temp_dnpc].st_name);
    print_depth--;
  }
  //打印格式为：【当前函数名】0x当前地址：深度 ret 【返回函数名@跳转地址】
  else{  //否则是调用指令
    printf("0x%0x:", addr);
    for(int i=0;i<=print_depth;i++)
      printf(" ");
    printf("[%s]call [%s@0x%0x]\n",strtab+symtab[temp_pc].st_name,strtab+symtab[temp_dnpc].st_name,next);
    print_depth++;
  }
  //打印格式为：【当前函数名】0x当前地址：深度 call 【调用函数名@跳转地址】
}
