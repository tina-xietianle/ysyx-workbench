...
#define RESET_VECTOR (PMEM_LEFT + CONFIG_PC_RESET_OFFSET) //0x80000000+0
...
static inline bool in_pmem(paddr_t addr) {
  return addr - CONFIG_MBASE < CONFIG_MSIZE;//用输入的地址减去内存的首地址，要是小于内存的大小，则该地址在内存内(in pmem)，即可以访问到
}
